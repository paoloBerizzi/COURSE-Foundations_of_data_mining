...
if __name__=='__main__':
    ...
    print(confusion_matrix(predictions.flatten().reshape(-1,1),y_test))
    
    
#    accuracy_mtx = np.zeros((lenlr,lenWeigthsvecto))
#    for comb in combinations:
    
        # comb = (mean,std,lr)
#        mlp = MLP(mean,std,lr)--> weights = ...np.random.normal(mean,std)
#        mlp.fit()
#        accuracy <--- mlp.predict()
#         accuracy_mtx[idx1,idx2] = accuracy
    #end for
